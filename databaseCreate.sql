...
create table recipe_category (recipe_id bigint not null auto_increment, category_id bigint not null, primary key (recipe_id, category_id)) engine=InnoDB;
create table unit_of_measure (id bigint not null auto_increment, description varchar(255), primary key (id)) engine=InnoDB;
alter table ingredient add constraint ingrec foreign key (recipe_id) references recipe (id);
alter table ingredient add constraint inguni foreign key (uom_id) references unit_of_measure (id);
alter table notes add constraint notuni foreign key (recipe_id) references recipe (id);
alter table recipe add constraint recnot foreign key (notes_id) references notes (id);
alter table recipe_category add constraint reccat foreign key (category_id) references category (id);
alter table recipe_category add constraint reccrec foreign key (recipe_id) references recipe (id);
